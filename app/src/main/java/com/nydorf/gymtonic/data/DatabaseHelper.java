...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    /**
     * This method will be called upon creation of the database. This method will
     * create all the
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        sqLiteDatabase.execSQL("CREATE TABLE " + TABLE_WR + " (" + COL_WR_PLAN_NAME + " TEXT, " + COL_WR_ROUTINE_NAME
                + " TEXT, PRIMARY KEY (" + COL_WR_PLAN_NAME + ", " + COL_WR_ROUTINE_NAME + "));");
        ...
        // -> ('index', 'name', 'category', cal, fat, fatsat, carbs, sugar, protein,
        // salt, fiber, cholesterin, creatine, ca, fe, k_potassium, mg, mn, na_sodium,
        // phosphor, zn, vita, vitb1, b2, b3, b5, b6, b7, b11, b12, c, e, k, h)
        // -> Preset meals indices have always 1 digit more (7 digits in total) than
        // user created meals to prevent overlaps
        sqLiteDatabase.execSQL("INSERT INTO " + TABLE_PM
                + " VALUES('000000000', 'Apple (100 g)', 'Fruits and Vegetables', 52, 0.17, 0, 13.81, 10.39, 0.26, 0, 2.4, 0, 0, 6, 0.12, 107, 5, 0.035, 1, 11, 0.04, 0.003, 0.017, 0.026, 0.091, 0.061, 0.041, 0, 0, 0, 4.6, 0.18, 0.022, 0)");
        sqLiteDatabase.execSQL("INSERT INTO " + TABLE_PM
                + " VALUES('000000001', 'Banana (100 g)', 'Fruits and Vegetables', 95.0, 0.33, 0.0, 22.84, 12.23, 1.0, 0.0, 2.6, 0.0, 0.0, 5.0, 0.26, 358.0, 27.0, 0.0, 0.0, 22.0, 0.15, 0.003, 0.031, 0.073, 0.665, 0.334, 0.367, 0.0, 0.0, 0.0, 8.7, 0.0, 0.0, 0.0)");
        ...
        // Workout exercises
        sqLiteDatabase.execSQL(
                "INSERT INTO " + TABLE_WE + " VALUES('Example Workout Plan', 'Push Day', 'Chest Press', 3, 6, 20)");
        sqLiteDatabase.execSQL(
                "INSERT INTO " + TABLE_WE + " VALUES('Example Workout Plan', 'Push Day', 'Shoulder Press', 3, 6, 20)");
        sqLiteDatabase.execSQL(
                "INSERT INTO " + TABLE_WE + " VALUES('Example Workout Plan', 'Pull Day', 'Lat Pull Down', 3, 8, 20)");
        sqLiteDatabase.execSQL(
                "INSERT INTO " + TABLE_WE + " VALUES('Example Workout Plan', 'Leg Day', 'Leg Press', 3, 6, 20)");

        // Settings
        // -> (index, calories, carbs, fat, protein). First value is index. Must always
        // be 0.
    ...
    /**
     * This method will be called upon upgrading the database from one version to a
     * higher one.
     *
     * @param sqLiteDatabase: SQLiteDatabase; The SQLiteDatabase to upgrade.
     * @param oldVersion:     Integer; The old version number of the database to
     *                        upgrade from.
     * @param newVersion:     Integer; The new version number of the database to
     *                        upgrade to.
     */
    @Override
    ...
    public Cursor getPresetMealsSimpleAllCategories() {
        ...
        if (sqLiteDatabase != null) {
            cursor = sqLiteDatabase.rawQuery(
                    "SELECT " + COL_PM_INDEX + ", " + COL_PM_NAME + ", " + COL_PM_CALORIES + " FROM " + TABLE_PM
                            + " ORDER BY " + COL_PM_NAME + " ASC LIMIT 100;",
                    null);
        }
    ...
    public Cursor getPresetMealsSimpleFromCategory(String category) {
        // -> Return index, name, calories from specified category table "foods" in
        ...
        if (sqLiteDatabase != null) {
            cursor = sqLiteDatabase.rawQuery(
                    "SELECT " + COL_PM_INDEX + ", " + COL_PM_NAME + ", " + COL_PM_CALORIES + " FROM " + TABLE_PM
                            + " WHERE " + COL_PM_CATEGORY + "='" + category + "'" + " ORDER BY " + COL_PM_NAME
                            + " ASC;",
                    null);
        }
    ...
    public Cursor getPresetMealDetails(String foodUUID) {
        ...
        if (sqLiteDatabase != null) {
            cursor = sqLiteDatabase
                    .rawQuery("SELECT * FROM " + TABLE_PM + " WHERE " + COL_PM_INDEX + "='" + foodUUID + "';", null);
        }
    ...
    public Cursor getPresetMealCategories() {
        ...
        if (sqLiteDatabase != null) {
            cursor = sqLiteDatabase.rawQuery(
                    "SELECT DISTINCT " + COL_PMC_NAME + " FROM " + TABLE_PMC + " ORDER BY " + COL_PMC_NAME + " ASC;",
                    null);
        }
    ...
    public Cursor getConsumedMeals(String date) {
        // -> Returns index, name, calories, amount for all consumed meals for a given
        // date
        ...
        if (sqLiteDatabase != null) {
            // SELECT food.food_index, food.name, food.calories, dailymeals.amount FROM
            // dailymeals LEFT JOIN food ON dailymeals.food_index=food.food_index WHERE
            // dailymeals.date=date;
            // Returns -> | food.food_index | food.name | food.cal | dailymeals.amount |
            cursor = sqLiteDatabase.rawQuery(
                    "SELECT " + TABLE_PM + "." + COL_PM_INDEX + ", " + TABLE_PM + "." + COL_PM_NAME + ", " + TABLE_PM
                            + "." + COL_PM_CALORIES + ", " + TABLE_CM + "." + COL_CM_AMOUNT + " " +
                            "FROM " + TABLE_CM + " " +
                            "LEFT JOIN " + TABLE_PM + " ON " + TABLE_CM + "." + COL_CM_INDEX + "=" + TABLE_PM + "."
                            + COL_PM_INDEX + " " +
    ...
    public Cursor getConsumedMealsSums(String date) {
        ...
        if (sqLiteDatabase != null) {
            ...
            // SELECT (food.calories * dailymeals.amount) [other columns here] FROM
            // dailymeals LEFT JOIN food ON dailymeals.food_index=food.food_index WHERE
            // dailymeals.date=date;
            // Returns -> | food.food_index | food.name | food.cal | food.fat | food.fat_sat
            // | food.carbs | food.sugar | food.protein | dailymeals.amount |
            cursor = sqLiteDatabase.rawQuery(
                    "SELECT " +
    ...
    public void removeConsumedMeal(String date, String mealUUID) {
        ...
        SQLiteDatabase sqLiteDatabase = this.getReadableDatabase();
        if (sqLiteDatabase != null) {
            sqLiteDatabase.delete(TABLE_CM, COL_CM_INDEX + "= ? AND " + COL_CM_DATE + "= ?",
                    new String[] { mealUUID, date });
            // sqLiteDatabase.delete(TABLE_NAME_M, COLUMN_M_INDEX + "='" + mealUUID + "' AND
            // " + COLUMN_M_DATE + "='" + date + "'", null);
            // sqLiteDatabase.rawQuery("DELETE FROM " + TABLE_NAME_M + " WHERE " +
            // COLUMN_M_INDEX + "='" + mealUUID + "' AND " + COLUMN_M_DATE + "='" + date +
            // "';", null);
        }
    }
    ...
    public Cursor getWorkoutRoutines(String planName) {
        ...
        if (sqLiteDatabase != null) {
            // cursor = sqLiteDatabase.rawQuery("SELECT DISTINCT " + COL_WE_ROUTINE_NAME + "
            // FROM " + TABLE_WE + " WHERE " + COL_WE_PLAN_NAME + "='" + plan +"';", null);
            cursor = sqLiteDatabase.rawQuery("SELECT DISTINCT " + COL_WR_ROUTINE_NAME + " FROM " + TABLE_WR + " WHERE "
                    + COL_WR_PLAN_NAME + "='" + planName + "';", null);
        }
    ...
    public Cursor getWorkoutExercises(String plan, String routine) {
        ...
        if (sqLiteDatabase != null) {
            cursor = sqLiteDatabase.rawQuery(
                    "SELECT " + COL_WE_EXERCISE_NAME + ", " + COL_WE_SETS + ", " + COL_WE_REPETITIONS + ", "
                            + COL_WE_WEIGHT +
                            " FROM " + TABLE_WE +
    ...
    public void addWorkoutExercise(String planName, String routineName, String exerciseName, int sets, int repetitions,
            double weight) {
        // Get database
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
    ...
    public void updateWorkoutRoutineName(String planName, String oldRoutineName, String newRoutineName) {
        ...
        if (sqLiteDatabase != null) {
            ...
            sqLiteDatabase.update(TABLE_WR, cv, COL_WR_PLAN_NAME + "= ? AND " + COL_WR_ROUTINE_NAME + "= ?",
                    new String[] { planName, oldRoutineName });
    ...
    public void deleteWorkoutRoutine(String planName, String routineName) {
        ...
        sqLiteDatabase.delete(TABLE_WR, COL_WR_PLAN_NAME + "= ? AND " + COL_WR_ROUTINE_NAME + "= ?",
                new String[] { planName, routineName });

        // Remove all entries for this routine in table "exercises"
        sqLiteDatabase.delete(TABLE_WE, COL_WE_PLAN_NAME + "= ? AND " + COL_WE_ROUTINE_NAME + "= ?",
                new String[] { planName, routineName });
    }
    ...
    public void deleteWorkoutExercise(String planName, String routineName, String exerciseName) {
        ...
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        if (sqLiteDatabase != null) {
            sqLiteDatabase.delete(TABLE_WE,
                    COL_WE_PLAN_NAME + "= ? AND " + COL_WE_ROUTINE_NAME + "= ? AND " + COL_WE_EXERCISE_NAME + "= ?",
                    new String[] { planName, routineName, exerciseName });
            // sqLiteDatabase.rawQuery("DELETE FROM " + TABLE_NAME_WE + " WHERE " +
            // COL_E_PLAN_NAME + "='" + planName + "' AND " + COL_E_ROUTINE_NAME + "='" +
            // routineName + "' AND " + COL_E_EXERCISE_NAME + "='" + exerciseName +"';",
            // null);
        }
    ...
    public Cursor getSettingsGoals() {
        ...
        if (sqLiteDatabase != null) {
            cursor = sqLiteDatabase.rawQuery(
                    "SELECT " + COL_S_GOAL_CALORIES + ", " + COL_S_GOAL_FAT + ", " + COL_S_GOAL_CARBS + ", "
                            + COL_S_GOAL_PROTEIN +
                            " FROM " + TABLE_S_GOAL +
                            " WHERE " + COL_S_INDEX + "=0;",
                    null);
