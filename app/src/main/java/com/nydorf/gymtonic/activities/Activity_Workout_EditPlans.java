...
public class Activity_Workout_EditPlans extends AppCompatActivity
        implements Adapter_Item_General_001.Interface_Item_Edit {

    /**
     * This activity lets the user edit workout plans. Changes will be saved to the
     * database.
     *
     * - This activity is for changing values of an existing plan only. Creating new
     * plans happens
    ...
    public void onButtonRemoveClicked(int itemPosition) {
        if (plansList.size() == 1) {
            // Toast.makeText(getApplicationContext(), "You must have at least 1 workout
            // plan", Toast.LENGTH_SHORT).show();
            showDialogError();
            return;
        }
