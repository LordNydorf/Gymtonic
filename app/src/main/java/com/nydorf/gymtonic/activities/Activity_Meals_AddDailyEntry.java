...
public class Activity_Meals_AddDailyEntry extends AppCompatActivity
        implements Adapter_MealPresets.mealPresetItemInterface, AdapterView.OnItemSelectedListener {

    /**
     * This activity displays all meal-presets and lets the user add them to the
     * database
     * for the current day.
     */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ArrayAdapter adapterCategories = new ArrayAdapter(getApplicationContext(), R.layout.spinner_item_purple_dark,
                mealCategories);
        ...
        recyclerViewMeals.setLayoutManager(
                ...
                if (savePossible) {
                    ...
                    for (Item_MealPreset currentMeal : mealsPresetList) {
                        if (currentMeal.getAmount() > 0) {
                            databaseHelper.addOrReplaceConsumedMeal(date, currentMeal.getMealUUID(),
                                    currentMeal.getAmount());
                        }
                    }
